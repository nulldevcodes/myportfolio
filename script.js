document.addEventListener('DOMContentLoaded', () => {
    ...
    let hasInteracted = false; // To track user interaction for autoplay

    // Function to attempt playing music
    function attemptPlayMusic() {
        if (musicPlayer && musicPlayer.paused) {
            musicPlayer.play().then(() => {
                updateControls(false); // Music is playing
            }).catch(error => {
                console.warn("Background music autoplay was prevented:", error);
                // Autoplay was prevented, user needs to click to start
                updateControls(true); // Music is paused
            });
        } else if (musicPlayer && !musicPlayer.paused) {
             updateControls(false); // Music is already playing
        }
    }
    ...
    function updateControls(isPaused) {
        if (isPaused) {
            ...
            playIcon.classList.add('icon-current');
            pauseIcon.style.display = 'none';
            pauseIcon.classList.remove('icon-current');
            playPauseBtn.setAttribute('aria-label', 'Play Background Music');
            playPauseBtn.setAttribute('title', 'Play Music');
            if (musicControlsContainer) musicControlsContainer.classList.remove('playing');
        } else {
            playIcon.style.display = 'none';
            playIcon.classList.remove('icon-current');
            pauseIcon.style.display = 'block';
            pauseIcon.classList.add('icon-current');
            playPauseBtn.setAttribute('aria-label', 'Pause Background Music');
            playPauseBtn.setAttribute('title', 'Pause Music');
            if (musicControlsContainer) musicControlsContainer.classList.add('playing');
        }
    }

    if (musicPlayer && playPauseBtn) {
        // Initial state based on player
        updateControls(musicPlayer.paused);
        ...
        // Optional: try to autoplay after a short delay once page is loaded
        // Browsers are very restrictive about autoplay with sound.
        // This might work if user has interacted with the site before or has lenient settings.
        // A click is usually required.
        window.addEventListener('load', () => {
            // Small timeout to let things settle
            setTimeout(() => {
                if (!hasInteracted) { // Only attempt if user hasn't manually clicked yet
                    // attemptPlayMusic(); // You can try enabling this, but it might be blocked.
                    // For a better UX, it's often best to wait for user click.
                    console.log("Background music ready. Click play to start.");
                }
            }, 1000);
        });

        // Update controls if music ends (though it's looped, this is good practice)
        musicPlayer.addEventListener('ended', () => {
            updateControls(true);
        });

        // Update controls if state changes for other reasons
        musicPlayer.addEventListener('play', () => updateControls(false));
        musicPlayer.addEventListener('pause', () => updateControls(true));

    } else {
        console.warn("Music player or control button not found.");
        if(musicControlsContainer) musicControlsContainer.style.display = 'none'; // Hide controls if player missing
    }
